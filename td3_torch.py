...
class Agent:
    def __init__(self, actor_learning_rate, critic_learning_rate, input_dims, tau, env, gamma = 0.99, update_actor_interval = 2,
                    warmup = 1000, n_actions = 2, max_size = 100000, layer1_size = 256, layer2_size = 128, batch_size = 100, noise = 0.1):
    ...
    def choose_action(self, observation, validation = False):
        ...
        # we want to have exploration initally.
        ...
        # mu_prime = T.max(T.min(mu_prime, self.max_action), self.min_action)
        mu_prime = mu + T.tensor(np.random.normal(scale = self.noise), dtype = T.float).to(self.actor.device) # add noise
        mu_prime = T.clamp(mu_prime, self.min_action[0], self.max_action[0]) # since action space os continuous
    ...
    def remember(self, state, action, reward, next_state, done):
        return self.memory.store_transition(state, action, reward, next_state, done)
    ...
    def learn(self):
        ...
        # target network takes in the next state and gives us target actions.
        # we add noise to make it explore and clamp it so that continuous actions are taken care of.
    ...
    # create update_network_parameters(tau = 1)
    def update_network_parameters(self, tau):
        '''
        some models use same parameters of the actor and critic to update the target networks.
        But here, I will use poliac averaging.
        '''
        if (tau == None):
            tau = self.tau
